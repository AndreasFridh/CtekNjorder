...
class Balancer:
    ...
    def compute(
        ...
        return self.from_headroom(now, headroom, house_age, charger_current,
                                  ev_uses_phase, baseline)
    ...
    def from_headroom(
        ...
        """
        Everything `compute` does except working out the headroom itself.

        Split out so a caller that arrives at the headroom another way - by
        regulating the meter reading directly, rather than by subtracting the
        cars out of it - still gets the cold start, the raise delay, the settle
        guard, the legal-value snapping and the stale-data fallback. Those are
        the safety-carrying parts and there should be exactly one copy of them.
        """
        cfg = self.cfg
        if not headroom or data_age > cfg.stale_timeout:
            why = "no house data" if not headroom else f"house data {data_age:.0f}s old"
            self._raise_pending_since = None
            return self._commit(
                now, cfg.fallback_current, f"FALLBACK: {why}",
                [float(cfg.fallback_current)] * 3, [],
